...
| Utrecht Toen (historische foto's op de kaart, al wandelend) | `utrecht-toen/` | geen tabel — de kaart is de staat | **andere map**: `~/Code/utrecht-in-beeld` (Next.js/D1-app met reviewwerkbank; geen git) | `node tools/build-utrecht-toen.mjs` voegt de twee JSON-snapshots daar samen tot `data.json` (132 foto's, zelfde dedup-regel als `lib/photo-repository.ts`) en controleert de invariantcounts uit die CLAUDE.md. MapLibre 5 van cdnjs, basemap OpenFreeMap (positron/dark volgt het thema). Alleen de publiekskaart; reviewen gebeurt in de bronrepo |
