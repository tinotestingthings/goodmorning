...
const mobileTabs: View[] = ["discover", "inbox", "saved", "timeline"];
...
export function EventTracker({ initialEvents, sources: initialSources, scheduledActions, monitoredFilms }: { initialEvents: EventRecord[]; sources: SourceRecord[]; scheduledActions: ScheduledAction[]; monitoredFilms: MonitoredFilm[] }) {
  ...
  const dropRegion = (value: string) => { setRegions((items) => items.filter((item) => item !== value)); removeRegion(value); showToast(`${value} removed from regions`); };
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><span className="brand-mark">EI</span><div><strong>Event Intelligence</strong><small>{regionLabel.toUpperCase()}</small></div></div>
        ...
        <div className="page">
          ...
          {(["discover", "inbox", "saved", "archive"] as View[]).includes(view) && <>
            ...
            <section className="event-grid" aria-live="polite">
              {filtered.map((event) => <EventCard key={event.id} event={event} onSave={() => changeState(event.id, event.state === "saved" ? "seen" : "saved")} onDismiss={() => dismiss(event)} onOpen={() => setDetail(event)} onRestore={() => changeState(event.id, "seen")} />)}
  ...
  return <article className="event-card">
    ...
    <div className="card-actions">
      {event.state === "dismissed"
        ...
        : <><button className={event.state === "saved" ? "secondary saved" : "secondary"} onClick={onSave}>{event.state === "saved" ? <><Icon name="check" size={14} /> Saved</> : <><Icon name="plus" size={14} /> Save</>}</button><button className="ghost" onClick={onDismiss}>Dismiss</button></>}
...
function Settings({ sources, onToggle, monitoredFilms, preferences, regions, onAddPreference, onAddRegion, onRemovePreference, onRemoveRegion }: { sources: SourceRecord[]; onToggle: (id: string) => void; monitoredFilms: MonitoredFilm[]; preferences: string[]; regions: string[]; onAddPreference: () => void; onAddRegion: () => void; onRemovePreference: (value: string) => void; onRemoveRegion: (value: string) => void }) {
  ...
  return <div className="settings-grid">
    <SettingsCard title="Regions" hint="Utrecht is home base. The catalogue currently covers Utrecht plus national film releases; extra regions are noted for future coverage.">
