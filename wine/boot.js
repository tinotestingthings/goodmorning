(function () {
  ...
  // WijnWijs auth gate + Supabase sync, with strict live/sandbox isolation.
  //
  // The wine app is a self-contained bundle that persists to localStorage under
  // a logical "wijnwijs-*" key. Live (served from the repo root) and the sandbox
  ...
  // live wine progress, this boot layer namespaces every key by environment — exactly
  ...
  var LOGICAL = "wijnwijs";        // what the app uses (key: wijnwijs-v1)
  var PHYS = NS + LOGICAL;         // what actually gets stored ("dd:wijnwijs-*" / "sbx:wijnwijs-*")
  ...
  function warn() { try { console.warn.apply(console, ["[WijnWijs]"].concat([].slice.call(arguments))); } catch (e) {} }
  ...
  function patchStorage() {
    if (proto.__winePatched) return;
    proto.__winePatched = true;
  ...
  var GATE_TITLE = "Sign in to use WijnWijs";
  var GATE_MSG = "Open the Daily Digest app and sign in — the wine trainer unlocks with your account.";
